...
```{r}
# might take a while
# NEI <- readRDS('data/summarySCC_PM25.rds'); SCC <- readRDS('data/Source_Classification_Code.rds')
```
...
pm25.agg.regr <- lm(x ~ year, pm25.agg)
...
```{r}
pm25.orioles <- aggregate(Emission ~ year, data=NEI, FUN=sum, subset = NEI$fips == '24510')
plot(Emissions ~ year, data=pm25.orioles)
# regression line
pm25.ori.regr <- lm(x ~ year, pm25.orioles)
# width=1, dotted, green
abline(pm25.ori.regr, lwd=1, lty=2, col='green')
```
...
```{r}
library(dplyr)
dt.balt <- tbl_df(pm25.orioles)
dt.balt.bytype <- dt.balt %>% group_by(type, year) %>% summarize(emissions=sum(Emissions)) %>% print

library(ggplot2)
qplot(year, emissions, data=dt.balt.sum, color=type, geom = c('point', 'smooth'))
```
